...
outDir <- "Z:/ayesha/USG_Collaboration/Analysis/figures/dataplots/183Sp_diffuse_rail4m/"
...
data3 <- read.csv(inpDir3, stringsAsFactors=FALSE)[,1:22]#split of empty last column
...
DFben <- data.frame(dataAll$Mine[1:28], dataAll$Len[1:28],bioBen3port,bioBen3portHomog, bioBen3portdirect, bioBen3portPer, bioBen3portHomogPer, finBen3port, finBen3portPer, numMine3port, numMine3portF, bioNorm3port3, bioNorm3port1, finNorm3port)
...
#######################
#Pannell plot with data
#######################
p2 <- ggplot(DFben, aes(x=finBen3portPer, y=bioBen3port)) +
	geom_blank() +
	theme_bw(17) + #remove grey #remove grids
	theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
	#scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0)) #make start at zero
	coord_cartesian(xlim=c(15,100), ylim=c(0,0.60001))+ #set x and y limits
	labs(x="Averted capital cost (% of total)", y="Averted biodiversity loss (number of species)")+
...
p <- ggplot(DFHetero, aes(Len, Impact, shape=HetorHom, colour=HetorHom)) +
	...
	scale_colour_manual(name="Distribution of biodiversity", breaks=c("Homog", "Hetero"), labels=c("Homogeneous", "Heterogeneous"), values=c("black", "grey70"))+ #change point colours and legend titles
	scale_shape_manual(name="Distribution of biodiversity", breaks=c("Homog", "Hetero"), labels=c("Homogeneous", "Heterogeneous"), values=c(19, 17))+ #change point shapes and make it a single legend
	theme_bw(17) + #get rid of grey bkg and gridlines
	theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
	coord_cartesian(xlim=c(0,650), ylim=c(0,0.75))+ #set x and y limits
	labs(x="Infrastructure length (km)", y="Biodiversity loss (number of species)")+
	theme(axis.title.x = element_text(vjust=-0.6),axis.title.y = element_text(vjust=1))+	#move xylabels away from graph
...
p1 <- ggplot(DFben, aes(x=numMine3portF, y=bioBen3portPer)) +
	...
	labs(x="Number of companies collaborating", y="Averted biodiversity loss (% of total)")+
...
ggplot(DFShareUn)+
	...
	labs(x="Capital cost (million $)", y="Biodiversity loss (number of species)")+
	theme_classic(17) + #get rid of grey bkg and gridlines
	#theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
	theme(axis.title.x = element_text(vjust=-0.6),axis.title.y = element_text(vjust=1))+	#move xylabels away from graph
	#edit legend
	scale_fill_discrete(breaks=c("UnshareSpLoss", "ShareSpLoss"))+
	scale_color_discrete(name="Scenario", labels=c("Non-collaborative", "Collaborative"),breaks=c("UnshareSpLoss", "ShareSpLoss"))+
	theme(legend.position=c(.25, .8))+#legend position
	theme(legend.background = element_rect(colour="grey70", size=.5, linetype="solid"))#add box
